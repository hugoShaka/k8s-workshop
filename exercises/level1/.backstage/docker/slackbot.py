...
def main():
    ...
    answer = int(os.getenv("ANSWER_TO_THE_ULTIMATE_QUESTION_OF_LIFE_THE_UNIVERSE_AND_EVERYTHING"))

    if slack_token is None:
        raise Exception("SLACK_TOKEN not defined")
    if slack_channel is None:
        raise Exception("SLACK_CHANNEL not defined")
    if username is None:
        raise Exception("YOUR_NAME not defined")
    if answer is None:
        raise Exception("ANSWER_TO_THE_ULTIMATE_QUESTION_OF_LIFE_THE_UNIVERSE_AND_EVERYTHING not defined")
    ...
    while True:
        if answer != 42:
            log.error("Wrong Answer to the Ultimate Question of Life, The Universe, and Everything was given.")
